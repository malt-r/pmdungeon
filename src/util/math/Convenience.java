...
public class Convenience {
   ...
   */
  public static Vec scaleDelta(Vec p1, Vec p2, float scale) {
   ...
   * @return A Vec of the length specified by scale
   */
  public static Vec scaleDelta(Point p1, Point p2, float scale) {
   ...
   */
  public static Point getFlooredPoint(Point toFloor) {
   ...
   */
  public static boolean areFlooredPointsEqual(Point p1, Point p2) {
   ...
   */
  public static boolean checkForIntersection(IDrawable drawable1, IDrawable drawable2) {
   ...
   * @param p1 first point.
   * @param p2 second point.
   * @param maxDiff The maximum difference between the two points which qualifies as "intersecting
   *     range"
   * @return if the two Points are in intersecting range
   */
  public static boolean checkForIntersection(Point p1, Point p2, float maxDiff) {
   ...
   * @return if the two Points are in intersecting range
   */
  public static boolean checkForIntersection(Point p1, Point p2) {
   ...
   * @param maxDiff The maximum difference between the two drawables which qualifies as
   *     "intersecting range"
   * @return if the two Points are in intersecting range
   */
  public static boolean checkForIntersection(
      IDrawable drawable1, IDrawable drawable2, float maxDiff) {
    var diff =
        new Vec(drawable1.getPosition()).subtract(new Vec(drawable2.getPosition())).magnitude();
    return diff < maxDiff;
  }
   ...
   */
  public static Tile convertPointToTile(Point p, DungeonWorld level) {
    int x = (int) p.x;
    int y = (int) p.y;
