...
public class Hero extends Actor implements items.IInventoryOpener, ObservableHero {
  ...
  private final float itemAddDamage = 0.0f;
  private final float itemAddDefence = 0.0f;
  private final float bonusHealth = 0.0f;
  private final float bonusDamage = 0.0f;
  private final Level level;
  private final ArrayList<HeroObserver> observerList = new ArrayList<HeroObserver>();
  private final ArrayList<HeroObserver> observersToRemove = new ArrayList<>();
  ...
  public Shield getLeftHandSlot() {
    return leftHandSlot;
  }

  public Weapon getRightHandSlot() {
  ...
  public Point lowerAttackRangeBound(Point ownPosition) {
    return new Point(
        (float) Math.floor(ownPosition.x) - getRangeOfWeapon(),
        (float) Math.floor(ownPosition.y) - getRangeOfWeapon());
  }

  @Override
  public Point upperAttackRangeBound(Point ownPosition) {
    return new Point(
        (float) Math.ceil(ownPosition.x) + 1 + getRangeOfWeapon(),
        (float) Math.ceil(ownPosition.y) + 1 + getRangeOfWeapon());
  }
  ...
  private void grantAbility() {
    ...
    switch (this.level.getCurrentLevel()) {
      case 2:
        this.level.addAbility(
            new SprintAbility(() -> Gdx.input.isKeyJustPressed(Input.Keys.SHIFT_LEFT)));
  ...
  private void applyLevelUp() {
    ...
    mainLogger.info(
        "Your base attack damage is now "
            + this.stats.getValue(AttributeType.PHYSICAL_ATTACK_DAMAGE));
  ...
  public void dealDamage(float damage, ICombatable attacker) {
    ...
    mainLogger.info(this + ": " + this.getHealth() + " health left");
  ...
  protected void generateAnimations() {
    ...
    String[] hitLeftFrames = new String[] {"tileset/hero/knight_m_idle_left_anim_f0_hit.png"};
    hitAnimationLeft = createAnimation(hitLeftFrames, 1);

    String[] hitRightFrames = new String[] {"tileset/hero/knight_m_idle_anim_f0_hit.png"};
  ...
  protected Point readMovementInput() {
    ...
    if (Gdx.input.isKeyPressed(Input.Keys.W)) {
      ...
      if (movementLog[0]) {
        mainLogger.fine("W press stopped");
      }
      movementLog[0] = false;
    }
    if (Gdx.input.isKeyPressed(Input.Keys.S)) {
      ...
      if (movementLog[1]) {
        mainLogger.fine("S press stopped");
      }
      movementLog[1] = false;
    }
    if (Gdx.input.isKeyPressed(Input.Keys.D)) {
      ...
      if (movementLog[2]) {
        mainLogger.fine("D press stopped");
      }
      movementLog[2] = false;
    }
    if (Gdx.input.isKeyPressed(Input.Keys.A)) {
      ...
      if (movementLog[3]) {
        mainLogger.fine("A press stopped");
      }
      movementLog[3] = false;
    }
  ...
  public void visit(Weapon weapon) {
    ...
    if (rightHandSlot != null) {
      inventory.addItem(rightHandSlot);
    }
  ...
  public void visit(Shield shield) {
    ...
    if (leftHandSlot != null) {
      inventory.addItem(leftHandSlot);
    }
    leftHandSlot = shield;
