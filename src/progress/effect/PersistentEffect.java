...
public abstract class PersistentEffect extends Effect {
    ...
    /**
     * Returns the Callable removalCheck. Is used to check, if the Effect should be scheduled for removal.
     * @return The Callable to execute as a removalCheck.
     */
    protected abstract Callable<Boolean> getRemovalCheck();

    /**
     * Should be called periodically. Updates internal state of the effect and performs checks, if the effect should be scheduled for removal.
     * @param target The target Actor, which should be affected.
     */
