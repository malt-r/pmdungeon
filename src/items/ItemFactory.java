...
public class ItemFactory {
  ...
  public static Item createItem(ItemType itemType) throws Exception {
    if (itemType == ItemType.SWORD_REGULAR) {
      return new RegularSword();
    }
    if (itemType == ItemType.SWORD_GOLD) {
      return new GoldenSword();
    }
    if (itemType == ItemType.POTION_HEAL) {
      return new HealthPotion();
    }
    if (itemType == ItemType.POTION_POISON) {
      return new PoisonPotion();
    }
    if (itemType == ItemType.SCROLL_ATTACK) {
      return new AttackScroll();
    }
    if (itemType == ItemType.SCROLL_SUPERVISION) {
      return new SupervisionScroll();
    }
    if (itemType == ItemType.SCROLL_SPEED) {
      return new SpeedScroll();
    }
    if (itemType == ItemType.SHIELD_WOOD) {
      return new WoodShield();
    }
    if (itemType == ItemType.SHIELD_EAGLE) {
      return new EagleShield();
    }
